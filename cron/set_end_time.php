<?php
    ...
    $endtime             = $objTaskcatReport->getzeroendtime();
    ...
    foreach($enddate as $enddt)
    {

        $task_end_time      = $enddt['task_end_datetime'];              // ISM ID
        $task_start_time    = $enddt['task_start_datetime'];            // SubCat ID
        $task_id            = $enddt['task_id'];


        $task_cat           = $enddt['task_cat'];              // ISM ID
        $task_subcat        = $enddt['task_subcat'];            // SubCat ID
        $task_ism           = $enddt['task_ism'];
        $st = explode(' ',$task_start_time);
        $create_date = $st[0];
