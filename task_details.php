<?php
    ...
    //Manual Task Assigning case start (dated = 2024-05-02) by Noman & Talha
    $manual_task_assigning   = "";
    $session_group_id        = @$_SESSION['group_id'];
    $session_group_id_array  = explode(",", $session_group_id);
    $existence = 0;

    if (in_array(38, $session_group_id_array)) {
        $existence = 1;
    }

    //Manual Task Assigning case end
    ...
    if(isset($_GET))
    {
        ...
        if($task_id > 0)
        {
            ...
            $dis_button             = ($data[0]['task_status_id'] == 3 || $data[0]['task_status_id'] == 4 || $data[0]['task_status_id'] == 6) ? "disabled='disabled'" : ""; //$data[0]['task_assigned_to'] != $login_id
            ...
            $manual_task_assigning = $data[0]['task_assigned_to'] == 0 ? ($existence ? '' : "disabled='disabled'") : '';
                ...
                <div class="panel-body">
                    <form class="form-horizontal" action="#" method="POST" id="task">
                                    ...
                                    <div class="form-group">
                                        <label>Assigned To<span style="color: red;">*</span></label>
                                            <!-- <?php if (($user_type == 1 || $user_type == 2) &&
                                                    $data[0]['task_status_id'] != 3 &&
                                                    $confirm_manual_assign[0]['operation_mode'] == 0) { ?>
                                            <select class="form-control default-select2" id="ddlAssignedTo" name="ddlAssignedTo" data-size="10" data-live-search="true" data-style="btn-white"<? echo $dis_button; ?> <?php echo $disable_task_progress; ?>  <?php echo $manual_task_assigning; ?>>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control default-select2" id="ddlProgress" name="ddlProgress" data-size="10" data-live-search="true" data-style="btn-white" <? echo $disable_task_progress; ?> >
                                            ...
                                            <!-- <option value="50" <?php echo $data[0]['task_status_id'] == '2' ? "selected" : '' ?> >In Progress</option>
                                            <option value="100" <?php echo $data[0]['task_status_id'] == '3' ? "selected" : '' ?> >Completed</option> -->
                                            ...
                                            <?php
                                                for($a=2; $a<count($files); $a++)
                                                {
                                                    $datas .='<td class="text-center">';
                                                        ...
                                                        $datas .='<div><a title="'.$files[$a].'" class="btn btn-primary btn-sm" href="'.$filepath7.$files[$a].'" download>'.$files[$a].'<a/></div>';
                                                    $datas .='</td>';
                                                }
                                                echo $datas;
                                            ?>
                        ...
                        <div class="col-md-12">
                            <div class="col-md-5">
                                <div class="form-group">
                                    <button type="button" class="btn btn-sm btn-primary" id="btnSubmitTask" <? echo $dis_button; ?> <?php echo $disable_task_progress; ?>  <?php echo $manual_task_assigning; ?>  data-loading-text="<i class='fa fa-spinner fa-spin '></i> Process...">Submit</button>
                                    <button type="button" class="btn btn-sm btn-primary" <? echo $dis_button; ?> <?php echo $disable_task_progress; ?>  <?php echo $manual_task_assigning; ?>  data-loading-text="<i class='fa fa-spinner fa-spin '></i> Process..." onclick="$('#ModalComment').modal('show'); $('#progress_status').val('In_Progress');">Upload File</button>
                                  <?php if($subtask_ism != 0 ){?>
                                    <a <? echo $dis_button; ?> <? echo $manual_task_assigning; ?> class="btn btn-danger btn-sm checkUpdate" href="task_add.php?tid=<?php echo $task_id ; ?>&pno=<?php echo $data[0]["policy_number"]?>"> Add SubTask <i class="glyphicon glyphicon-edit icon-white"></i></a>
                                <?php } ?>
                                <?php if( ($user_type == 1 || $user_type == 2) && $data[0]['task_status_id'] != 3 ) { ?>
                                    <?php if($confirm_manual_assign[0]['operation_mode']){ ?>
                                        <button type="button" class="btn btn-sm btn-primary" id="btnReasignTask" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Process..." onclick="reasignManual();">Reassign Manual</button> <!--  onclick="reasign();">Reassign Auto -->
                                    <?php } elseif ($confirm_manual_assign[0]['operation_mode'] == 0){ ?>
                                        <button type="button" class="btn btn-sm btn-primary" id="btnReasignTask" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Process..." onclick="reasignManual();">Reassign Manual</button>
                                    <?php } ?>
                                    <button type="button" class="btn btn-sm btn-primary" id="btnupReasignTask" style="display: none;margin-left: 238px;margin-top: -29px;" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Process..." onclick="reasignManual();">Update</button>
                                <?php } ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="panel-body">
                        <form role="form" autocomplete="off" method="post" class="form-horizontal" id="modalform" enctype="multipart/form-data" style="clear:both">
                            <fieldset>
                                ...
                                <input type="hidden" class="form-control" id="progress_status"  name="progress_status" value="In_Progress">
...
<script type="text/javascript">
    $("#btnSubmitTask").click(function () 
    {
        ...
        var assign_to = $('#ddlAssignedTo').val();
            ...
            $.ajax({
                ...
                success: function (data) 
                {
                    ...
                    if(data == 'success')
                    {
                        ...
                        $('#progress_status').val($('#ddlProgress option:selected').text().replace(/\s+/g, '_'));
                        
                        $('#task_num').val(<? echo $task_num ?>);
    ...
    $(document).on('click', '#btnFileUpload', function () {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                var message = "Task file uploaded successfully with Task Id <strong>";
                ...
                if(tempdata[0] == 'success')
                {
                    ...
                    $.notifyBar({ cssClass: "success", html: message +  <? echo $task_id ?> + "</strong>", delay: 2000, animationSpeed: "normal" });
                    setTimeout(function () {
    ...
    function reasignManual()
    {
        var task    = $('#txtId').val();
        var assignedTo    = $('#ddlAssignedTo').val();
        var action  = 'reasign_manual_user';
        ...
        $.ajax({
            ...
            success: function(data) 
            {
                ...
                else if(data == 'fail')
                {
                    $('#txtEmail').addClass('error-val');
                    $('#txtEmail').focus();
                    $.notifyBar({ cssClass: "warning", html: "Note: This task can't be reasigned", delay: 5000, animationSpeed: "normal" });
                }
            }
        });
    }
