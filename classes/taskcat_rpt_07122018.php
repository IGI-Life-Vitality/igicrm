...
class TaskcatReport
{
    ...
    function getIsmByDepartmentId($id,$duration,$month,$quarter,$year)
    {
        ...
        $query .= "SELECT ism.*, subcat.fullname AS subcat_name, wat.priority AS weightage FROM tbl_task_isam ism LEFT JOIN tbl_task_subcategory subcat ON subcat.id = ism.sub_cat_id LEFT JOIN tbl_priority wat ON wat.id = ism.priority LEFT JOIN tbl_task_new tnew ON tnew.task_ism = ism.id WHERE ism.is_major = '1' AND tnew.parent_task_id = '0' $data GROUP BY ism.id ORDER BY ism.id ASC";
    ...
    {
        $query = "SELECT COUNT(1) AS total_ism_where_task_closed FROM tbl_task_new tnew LEFT JOIN tbl_task_isam ism ON tnew.task_ism = ism.id WHERE tnew.group_id = '$group_id' AND tnew.parent_task_id = '0' AND tnew.task_status_id = '3' AND ism.is_major = '1' GROUP BY tnew.task_ism";
        //return $query;
        return $this->mysqli_lib->fetch_all($query);
    }
    ...
    function getTodayTask($ism_sub_cat_id,$ism_id,$present_date,$task_status)
    {
        ...
        $query = "SELECT COUNT(1) AS task_counts FROM tbl_task_new WHERE task_subcat = '$ism_sub_cat_id' AND task_ism = '$ism_id' AND DATE(task_create_date) = '$present_date' AND parent_task_id = '0' ORDER BY task_id ASC";
    ...
    {
        $query = "SELECT COUNT(1) AS task_counts FROM tbl_task_new WHERE task_subcat = '$ism_sub_cat_id' AND task_ism = '$ism_id' AND DATE(task_update_date) = '$present_date' AND task_status_id IN ($task_status) AND parent_task_id = '0' ORDER BY task_id ASC";
