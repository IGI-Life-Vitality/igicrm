...
class TaskcatReport
{
    ...
    function getIsmdetails($id,$duration,$month,$quarter,$year)
    {
        ...
        $query .= "SELECT ism.*, subcat.fullname AS subcat_name, wat.priority AS weightage, tnew.task_end_datetime AS task_end_datetime FROM tbl_task_isam ism LEFT JOIN tbl_task_subcategory subcat ON subcat.id = ism.sub_cat_id LEFT JOIN tbl_priority wat ON wat.id = ism.priority LEFT JOIN tbl_task_new tnew ON tnew.task_ism = ism.id WHERE 1=1 $data AND ism.isactive = '1' GROUP BY ism.id ORDER BY ism.id ASC";
    ...
    function getTaskDiff($ism_sub_cat_id,$ism_id,$present_date)
    {
        $query = "SELECT 
                    (CASE
                    ...
                    FROM tbl_task_new_rpt WHERE task_status_id IN (1,2,5,6,7) AND task_subcat = '$ism_sub_cat_id' AND task_ism = '$ism_id' AND task_date = '$present_date' GROUP BY DiffInDays";
    ...
    function getBFDetails($ism_sub_cat_id,$ism_id,$incomming_status,$present_date,$last_date)
    {
        $query = "SELECT 
                      COUNT(1) AS bf
                    FROM
    ...
    function GetBFDetailDateWise($ism_sub_cat_id,$ism_id,$incomming_status,$present_date,$last_date)
    {
        $query = "SELECT * FROM tbl_task_new WHERE task_subcat = '$ism_sub_cat_id' AND task_ism = '$ism_id' AND task_status_id IN ($incomming_status)";

        #`echo "$query |<br>" >> /tmp/log_cron_set_daily_tasks_query.log`;

        //return $query;
        $res = $this->mysqli_lib->fetch_all($query);

        $count = count($res);

        `echo "$query || $count |<br>" >> /tmp/log_cron_set_daily_tasks_query_count_$present_date.log`;
        foreach($res as $response)
        {
            $task_id = $response['task_id'];
            $counter = $response['task_daily_counter'];
            $group_id = $response['group_id'];
            $task_num = $response['task_num'];
            $task_cat = $response['task_cat'];
            $task_subcat = $response['task_subcat'];
            $task_ism_id = $response['task_ism'];
            $task_ism_desc = $response['task_ism_desc'];
            $task_title = $response['task_title'];
            $task_desc = $response['task_desc'];
            $login_id = $response['task_assignee'];
            $assigned_to = $response['task_assigned_to'];
            $verified_by = $response['task_verified_by'];
            $cc_users = $response['task_cc_list'];
            $start_time = $response['task_start_datetime'];
            $end_datetime = $response['task_end_datetime'];
            $priority = $response['task_priority'];
            $task_create_date = $response['task_create_date'];
            $task_update_date = $response['task_update_date'];
            $policy_number = $response['policy_number'];
            $sub_task_id = $response['sub_task_id'];
            $parent_task_id = $response['parent_task_id'];
            $is_complete = $response['is_complete'];

	    $query = "INSERT INTO tbl_task_new_rpt(task_id,task_daily_counter, group_id ,task_num,task_cat,task_subcat,task_ism,task_ism_desc,task_title,task_desc,task_assignee,task_assigned_to,task_verified_by,task_cc_list,task_start_datetime,task_end_datetime,task_priority,task_create_date,task_update_date,policy_number,sub_task_id,parent_task_id,is_complete,task_date) VALUES ('".$task_id."','".$counter."', '".$group_id."' ,'".$task_num."','".$task_cat."','".$task_subcat."','".$task_ism_id."','".$task_ism_desc."','".$task_title."','".$task_desc."','".$login_id."','".$assigned_to."','".$verified_by."','".$cc_users."','".$start_time."','".$end_datetime."','".$priority."','".$task_create_date."','".$task_update_date."', '".$policy_number."','".$sub_task_id."','".$parent_task_id."','".$is_complete."','".$present_date."')";

	     $response = $this->mysqli_lib->insert($query);

             `echo "$query || $response |<br>" >> /tmp/log_cron_set_daily_tasks_query_insert_$present_date.log`;
        }
   ...
   function getIsmdetailsNew()
    {
        
        $query = "SELECT ism.*, subcat.fullname AS subcat_name, wat.priority AS weightage FROM tbl_task_isam ism LEFT JOIN tbl_task_subcategory subcat ON subcat.id = ism.sub_cat_id LEFT JOIN tbl_priority wat ON wat.id = ism.priority  WHERE 1=1  AND ism.isactive = '1' GROUP BY ism.id ORDER BY ism.id ASC";
  ...
  function GetBFDetailDateWise_test($ism_sub_cat_id,$ism_id,$incomming_status,$present_date,$last_date)
    {
        $query = "SELECT * FROM tbl_task_new WHERE task_subcat = '$ism_sub_cat_id' AND task_ism = '$ism_id' AND task_status_id IN ($incomming_status)";
        ...
        foreach($res as $response)
        {
            ...
            $query = "INSERT INTO tbl_task_new_rpt_test(task_id,task_daily_counter, group_id ,task_num,task_cat,task_subcat,task_ism,task_ism_desc,task_title,task_desc,task_assignee,task_assigned_to,task_verified_by,task_cc_list,task_start_datetime,task_end_datetime,task_priority,task_create_date,task_update_date,policy_number,sub_task_id,parent_task_id,is_complete,task_date) VALUES ('".$task_id."','".$counter."', '".$group_id."' ,'".$task_num."','".$task_cat."','".$task_subcat."','".$task_ism_id."','".$task_ism_desc."','".$task_title."','".$task_desc."','".$login_id."','".$assigned_to."','".$verified_by."','".$cc_users."','".$start_time."','".$end_datetime."','".$priority."','".$task_create_date."','".$task_update_date."', '".$policy_number."','".$sub_task_id."','".$parent_task_id."','".$is_complete."','".$present_date."')";
    ...
    function getzeroendtime()
    {
        
        $query = "SELECT task_id,task_start_datetime,task_end_datetime FROM tbl_task_new WHERE task_end_datetime LIKE '%00:00:00'";

        //return $query;
        return $this->mysqli_lib->fetch_all($query);
    }

    function SetEndTime($final_time,$task_id)
    {
        
        $query = "UPDATE tbl_task_new SET  task_end_datetime = '$final_time' WHERE task_id = '$task_id'";
        
        //return $query;
        $respone = $this->mysqli_lib->update($query);
        return $respone;
    }
   

   function getnintyseventytenddate()
    {

        $query = "SELECT task_id,task_start_datetime,task_end_datetime,task_cat,task_subcat,task_ism FROM tbl_task_new WHERE task_end_datetime LIKE '1970-01-01%'";
    ...
    function GetIsmDetail($task_id)
    {
        $query = "SELECT tat FROM tbl_task_isam WHERE id = '$task_id'";
