...
class Taskcat
{
    ...
    function SearchTask($login_id,$user_type,$group_id,$cat,$subcat,$ism,$status,$start_date,$end_date,$policy_num)
    {
        ...
        {
            $query .= "SELECT t.*, us.user_name AS assignedBy, usr.user_name 
                     AS assignedTo, pr.priority, usrs.user_name AS verifiedBy FROM tbl_task_new t INNER JOIN tbl_users us ON us.id = t.task_assignee INNER JOIN tbl_priority pr ON pr.id = t.task_priority INNER JOIN tbl_users usr ON usr.id = t.task_assigned_to INNER JOIN tbl_users usrs ON usrs.id = t.task_assigned_to WHERE t.group_id IN ($group_id) $data ORDER BY t.task_start_datetime DESC";
        }
        else
        {
