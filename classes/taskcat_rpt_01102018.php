...
class TaskcatReport
{
    ...
    function getTaskRating($ism_id,$duration,$month,$quarter,$year)
    {
        $query  = "";
        $data   = "";
        ...
        $query = "SELECT 
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_status_id = '3' AND task_update_date <= task_end_datetime $data) AS task_within_tat,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' $data) AS total_service_request,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_status_id = '3' $data) AS total_closed_service,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_create_date >= DATE_ADD(LAST_DAY(DATE_SUB(NOW(), INTERVAL 2 MONTH)), INTERVAL 1 DAY) AND task_create_date <= DATE_ADD(LAST_DAY(DATE_SUB(NOW(), INTERVAL 1 MONTH)), INTERVAL 1 DAY) $data) AS created_last_month,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_status_id = '3' AND task_update_date <= task_end_datetime AND task_create_date >= DATE_ADD(LAST_DAY(DATE_SUB(NOW(), INTERVAL 2 MONTH)), INTERVAL 1 DAY) AND task_create_date <= DATE_ADD(LAST_DAY(DATE_SUB(NOW(), INTERVAL 1 MONTH)), INTERVAL 1 DAY) $data) AS score_last_month,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_status_id = '3' AND MONTH(task_create_date) = MONTH(CURDATE()) $data) AS created_current_month,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_ism = '$ism_id' AND task_status_id = '3' AND task_update_date <= task_end_datetime AND MONTH(task_create_date) = MONTH(CURDATE()) $data) AS score_current_month";
        //return $query;
        return $this->mysqli_lib->fetch_all($query);
    }
    ...
    function getTaskCounts($ism_id)
    {
        ...
        $query .= "SELECT
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_status_id IN ('1','2','5','6','7') AND DATE(task_start_datetime) < DATE(NOW()) AND task_ism = '$ism_id' AND is_bf = '0') AS BF,
                    (SELECT COUNT(*) FROM tbl_task_new WHERE task_status_id = '3' AND DATE(task_start_datetime) < DATE(NOW()) AND is_bf = '1' AND task_ism = '$ism_id') AS BF_Closed,
    ...
    function saveIsmWiseResultDetails($ism_type,$ism_code,$tat,$avg_mint_activity,$bf,$incomming,$total,$today_completed,$pending_cf,$main_hours,$day1,$day2,$day3,$day4,$log_datetime)
    {
        $query  = "";
        $query .= "INSERT INTO rpt_task_ism_wise (ism_type,ism_code,tat,avg_mint_activity,bf,incomming,total,today_completed,cf,main_hours,day_1,day_2,day_3,day_4_above,log_datetime) VALUES ('$ism_type','$ism_code','$tat','$avg_mint_activity','$bf','$incomming','$total','$today_completed','$pending_cf','$main_hours','$day1','$day2','$day3','$day4','$log_datetime')";
