...
class User 
{
    ...
    function AddUsers($first_name,$last_name,$user_type,$user_id,$email,$password,$employee_id,$mobile,$location,$group_id,$date_time,$is_active)
    {
        ...
        $query = "INSERT INTO tbl_users (first_name,last_name,user_type,user_name,email,user_pass,employee_id,mobile_no,location_id,group_id,isactive,create_datetime,expiry_datetime,last_login) VALUES ('$first_name','$last_name','$user_type','$user_id','$email','$password','$employee_id','$mobile','$location','$group_id','$is_active',NOW(),'$date_time', '0000-00-00 00:00:00');";
        $response = $this->mysqli_lib->insert($query);

        return $response . "|" ."success";
    }

    function UpdateUsers($id,$first_name,$last_name,$user_type,$email,$mobile,$location,$date_time,$group_id,$is_active)
    {
        $firstname = ucwords($first_name);
        $group_id  = implode(",", $group_id);
        $date_time = date("Y-m-d H:i:s", strtotime($date_time));

        $query = "UPDATE tbl_users SET first_name = '$firstname' , last_name = '$last_name', user_type = '$user_type', email = '$email', mobile_no = '$mobile',location_id = '$location', expiry_datetime = '$date_time', group_id = '$group_id', isactive = '$is_active' WHERE id = '$id'";
        //return $query;
    ...
    {
        $query = "SELECT * FROM tbl_users WHERE FIND_IN_SET('$id', group_id) AND isactive = 1 AND id NOT IN ('1') ORDER BY Id";
        return $this->mysqli_lib->fetch_all($query);
    }

    function GetUsersByGroups($id) 
    {
        $query = "SELECT * FROM tbl_users WHERE FIND_IN_SET('$id', group_id) AND isactive = 1 AND user_type NOT IN ('1','2','3')  ORDER BY Id";
        return $this->mysqli_lib->fetch_all($query);
    }

    function GetUsersByGroupsForTask($id) 
    {
        $query = "SELECT * FROM tbl_users WHERE FIND_IN_SET('$id', group_id) AND isactive = 1 AND user_type NOT IN ('1')  ORDER BY Id";
        return $this->mysqli_lib->fetch_all($query);
    }
    ...
    {
        $query = "SELECT isactive FROM tbl_users WHERE user_name = '".$userid."'";
    ...
    function UpdateUserlogin($userid, $isactive)
    {
        $query_update = "UPDATE tbl_users SET last_login = NOW(),isactive = '$isactive' WHERE user_name = '".$userid."'";
        $this->mysqli_lib->update($query_update);
    }

    function UserBlock($userid, $isactive)
    {
        $query_update = "UPDATE tbl_users SET last_login = NOW(),isactive = '$isactive' WHERE user_name = '".$userid."'";
        $this->mysqli_lib->update($query_update);
    }
    ...
    function GetCheckUser($userid, $pass)
    {
        $query = "SELECT * FROM tbl_users WHERE user_name = '".$userid."' AND user_pass = '".$pass."'";
    ...
    function GetCheckUserExpiry($userid, $pass)
    {
        $query = "SELECT * FROM tbl_users WHERE user_name = '".$userid."' AND user_pass = '".$pass."' AND expiry_datetime > NOW()";
