...
class Taskcat
{
    ...
    {
        $query = "SELECT * FROM `tbl_task_isam` where task_category_id ='$task_category' and sub_cat_id = '$subcat' AND is_major IN ($is_main)";
        //return $query;
        return $this->mysqli_lib->fetch_all($query);
    }
    ...
    function SearchTask($login_id,$user_type,$group_id,$cat,$subcat,$ism,$status,$start_date,$end_date,$policy_num,$taskNumber)
    {
        $query = '';
        ...
        if($taskNumber != '')
        {
            $data .= " AND t.task_num = '$taskNumber'";
        }

        if($user_type == 1)
        {
            $query .= "SELECT t.*, us.user_name AS assignedBy, usr.user_name 
                     AS assignedTo, pr.priority, usrs.user_name AS verifiedBy FROM tbl_task_new t LEFT JOIN tbl_users us ON us.id = t.task_assignee LEFT JOIN tbl_priority pr ON pr.id = t.task_priority LEFT JOIN tbl_users usr ON usr.id = t.task_assigned_to LEFT JOIN tbl_users usrs ON usrs.id = t.task_assigned_to WHERE 1=1 $data ORDER BY t.task_start_datetime DESC";
        }
        else if($user_type == 2)
        {
            $query .= "SELECT t.*, us.user_name AS assignedBy, usr.user_name 
                     AS assignedTo, pr.priority, usrs.user_name AS verifiedBy FROM tbl_task_new t LEFT JOIN tbl_users us ON us.id = t.task_assignee LEFT JOIN tbl_priority pr ON pr.id = t.task_priority LEFT JOIN tbl_users usr ON usr.id = t.task_assigned_to LEFT JOIN tbl_users usrs ON usrs.id = t.task_assigned_to WHERE t.group_id IN ($group_id) $data ORDER BY t.task_start_datetime DESC";
        }
        else
        {
            //$query .= "SELECT t.*, us.user_name AS assignedBy, usr.user_name AS assignedTo, pr.priority, usrs.user_name AS verifiedBy FROM tbl_task_new t LEFT JOIN tbl_users us ON us.id = t.task_assignee LEFT JOIN tbl_priority pr ON pr.id = t.task_priority LEFT JOIN tbl_users usr ON usr.id = t.task_assigned_to LEFT JOIN tbl_users usrs ON usrs.id = t.task_assigned_to WHERE (t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id' OR t.task_verified_by = '$login_id') AND t.parent_task_id = '0' $data ORDER BY t.task_start_datetime DESC";

            $query .= "SELECT t.*, us.user_name AS assignedBy, usr.user_name AS assignedTo, pr.priority, usrs.user_name AS verifiedBy FROM tbl_task_new t LEFT JOIN tbl_users us ON us.id = t.task_assignee LEFT JOIN tbl_priority pr ON pr.id = t.task_priority LEFT JOIN tbl_users usr ON usr.id = t.task_assigned_to LEFT JOIN tbl_users usrs ON usrs.id = t.task_assigned_to WHERE (t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id' OR t.task_verified_by = '$login_id') $data ORDER BY t.task_start_datetime DESC";
        }
