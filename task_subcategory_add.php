...
if(isset($_GET))
{
    ...
    if($id > 0)
    {
        $data = $objTaskcat->GetSubcategoriesActive($id);
        $heading = "Edit SubCategory";
        $isactive = $data[0]['isactive'] == 1 ? "checked='checked'" : "";
        $title = "<li> Edit Subcategory </li>";
    ...
    else
    {
        ...
        $title .= "<li class='active'> Add Subcategory </li>";
    }
}
                ...
                <div class="panel-body">
                    <form class="form-horizontal" autocomplete="off" id="ProductTypeForm">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <select class="form-control default-select2" id="ddlTaskCategory" name="ddlTaskCategory" title="Please select">
                                    <option value="0">Select Task Category</option>
                                    <?php $task_categories = $objTaskcat->GetTaskCategory(0); ?>
                                    <?php foreach($task_categories as $task_category){ ?>
                                        <option value="<? echo $task_category["id"]; ?>"<?php echo ($data[0]["task_category"] == $task_category["id"] ? "selected='selected'" : ""); ?>><? echo $task_category["fullname"] ?></option>
                                    <? } ?>
                                </select>
                        ...
                        <div class="form-group">
                            <label class="col-md-2 control-label-my">Subcategory</label>
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="txtName" id="txtName" value="<?php echo($data[0]['fullname']); ?>" placeholder="Enter Subcategory Name" onkeypress="return validateAlphabets(event)" />
                                <div class="input-error form-control-input" style="color: Red; display: none;">Subcategory Name is required</div>
                            </div>
                        </div>
            ...
            if (validation()) {
                ...
                $.ajax({
                    data: {
                        ...
                        'id': id,
                        'fullname': name,
                        'product_category': task_category,
                        'isactive': is_active
                    },
                    type: 'POST',
                    ...
                    success: function (data) {
                        ...
                        if (data == 'success') {
                            ...
                            setTimeout(function () { window.location.href = "task_subcategory_list.php" }, 3000);
                        } else if (data == 'fail') {
