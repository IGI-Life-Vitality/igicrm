...
if(isset($_POST)) 
{
    ...
    if($action == 'save_complaint')
    {
        ...
        $policyIssuanceDate          = isset($_POST['policy_issuance_date'])?$_POST['policy_issuance_date']: '';
        $planNature          = isset($_POST['plan_nature'])?$_POST['plan_nature']: '';
        $statusOfPolicy          = isset($_POST['status_of_policy'])?$_POST['status_of_policy']: '';
        $userid                = isset($_POST['cmp_user'])?$_POST['cmp_user']: 0;
        $policynum             = isset($_POST['txtPolicyNumber'])?$_POST['txtPolicyNumber']: '0';
        $cnic                  = isset($_POST['txtCNIC'])?(formatCNIC($obj_mysql->escape($_POST['txtCNIC']))): '';
        $Tat                   = isset($_POST['txtComplaintTAT'])?$_POST['txtComplaintTAT']:'';
        $cust_name             = isset($_POST['txtCustomerName'])?($obj_mysql->escape($_POST['txtCustomerName'])): '';
        ...
        $description           = isset($_POST['txtDescription'])?($obj_mysql->escape($_POST['txtDescription'])): '';
        $callback_num          = isset($_POST['txtCallBack'])?$_POST['txtCallBack']: '';
        $office_addr           = isset($_POST['txtOfficeAddress'])?($obj_mysql->escape($_POST['txtOfficeAddress'])): '';
        
        $correspondenceAddress = isset($_POST['txtCorrespondenceAddress'])?($obj_mysql->escape($_POST['txtCorrespondenceAddress'])): '';
        ...
        $email                 = isset($_POST['txtEmail'])?($obj_mysql->escape($_POST['txtEmail'])): '';
        ...
        $received_date           = isset($_POST['received_date'])?$_POST['received_date']: 0;
        ...
        // Send Email When Complaint Locked
        if($type != 'internal')
        {
            if($response > 0)
            {
                // if($email == '')
                // {
                //     $email = "haroon.saeed@m3tech.com.pk";
                // }

                // $sb ="Dear Customer,<br/><br/>Thank you for contacting IGI Life Insurance. Your complaint has been registered and is in process. Your complaint reference No is:<b> $comp_num</b>, which should be used for further communication with respect to your complaint.<br /><br/>We look forward to serving you better and for any further assistance, you are welcome to contact our call center on UAN: <b>021-111 111 711</b> <br /><br /><br />";
    ...
    {
        $Id                    = isset($_POST['complaintId'])?$_POST['complaintId']: 0;
        $counter               = isset($_POST['txtCounter'])?$_POST['txtCounter']: '';
        $complaint_no          = isset($_POST['complaintNum'])?$_POST['complaintNum']: '';
        $group_id              = isset($_POST['cmp_user_group'])?$_POST['cmp_user_group']: 0;
        $policyIssuanceDate          = isset($_POST['policy_issuance_date'])?$_POST['policy_issuance_date']: '';
        $planNature          = isset($_POST['plan_nature'])?$_POST['plan_nature']: '';
        $statusOfPolicy          = isset($_POST['status_of_policy'])?$_POST['status_of_policy']: '';
    ...
    elseif($action == 'search_complaint')
    {
        ...
        $complaint_num        = isset($_POST['cmp_num'])?$_POST['cmp_num']: '';
        $complaint_type       = isset($_POST['comp_type'])?$_POST['comp_type']: '';
        $assigned_to          = isset($_POST['agent'])?$_POST['agent']: '';
        $status               = isset($_POST['cmp_status'])?$_POST['cmp_status']: '';
        $start_date           = isset($_POST['FromDate'])?$_POST['FromDate']: '';
        $end_date             = isset($_POST['ToDate'])?$_POST['ToDate']: '';
        $policy_num           = isset($_POST['policy_num'])?$_POST['policy_num']: '';
        $user_type  = $_SESSION['user_type'];
        
        $data = $objComplaint->getComplaintView($login_id,$user_type,$group_id,$cnic,$complaint_num,$complaint_type,$assigned_to,$status,$start_date,$end_date,$policy_num);
                    ...
                    <tbody>';
                            foreach($data as $row)
                            {
                                ...
                                $output .='<tr>';
                                    ...
                                    $output .='<td>'.$row["close_date"].'</td>';
                                    $output .='<td>'.$row["file_counter"].'</td>';
                                    if($row["status_id"] != '3'  && $user_type == '2'){
                                        $output .="<td><a class='btn btn-primary btn-xs checkUpdate' href='complaint_edit.php?id=".$row['complaint_id']."&cmode=".$row['type']."'>
    ...
    elseif($action == 'update_progress')
    {
        ...
        $premium_amount = isset($_POST['premium_amount'])?$_POST['premium_amount']: '';
        $amount_claimed = isset($_POST['amount_claimed'])?$_POST['amount_claimed']: '';
        ...
        if($cmode == "individual")
        {
            $res = $objComplaint->ProgressComplaintlife($login_id,$complaint_id,$progress,$notes,$cmode,$manual,$cmp_user_name,$cmp_user_group,$invalid,$user_id,$priority,$tat,$user,$new_user,$is_manual,$departmentName,$cmp_type,$refund_ammount,$premium_amount,$amount_claimed);
            $detail         = explode('|',$res);
            $cumtomer_email = $detail[1];
            $complaint_num  = $detail[2];
            $feedbackUrl=generateFeedbackUrl($cmode, $complaint_num);
            $result         = $detail[0];
            $cc_emails      = "";
            $subject        = "IGI Complaint";

            if($cumtomer_email != "" && $progress == '100')
            {
                $sb = "";
                $sb .= "Dear Customer,<br/><br/>";
                $sb .= "This is to inform you that your complaint no. : <b>$complaint_num</b> has been resolved at our end.<br/><br/>";
                $sb .= "We assure you of our best services and would request you to provide your feedback which will help us in improving our services and serve you better. ";
                $sb .= "In case of any dispute kindly contact us at your earliest.<br/><br/>";
                $sb .= "Please share your feedback by clicking the link below:<br/>";
                $sb .= "<a href='$feedbackUrl' target='_blank'>$feedbackUrl</a><br/><br/>";
                $sb .= "We look forward to serving you better and for any further assistance, you are welcome to contact our call center on UAN <b>021-111 111 711</b>.<br/><br/><br/>";
                $sb .= "Best Regards,<br /><br />";
                $sb .= "Customer Experience and Conservation Department<br/><br/>";
                $sb .= "Email: services.life@igi.com.pk<br/>";
                $sb .= "Web: www.igilife.com.pk<br/>";
                $sb .= "UAN Number: 021-111-111-711";
        ...
        elseif($cmode == "legal")
        {
            $res = $objComplaint->ProgressComplaintLegal($login_id,$complaint_id,$progress,$notes,$cmode,$manual,$cmp_user_name,$cmp_user_group,$invalid,$user_id,$priority,$tat,$user,$new_user,$is_manual,$departmentName,$cmp_type,$refund_ammount,$premium_amount,$amount_claimed);
        ...
        elseif($cmode == "bancaIndividual")
        {
            ...
            $res = $objComplaint->ProgressComplaintBanca($login_id,$complaint_id,$progress,$notes,$cmode,$manual,$cmp_user_name,$cmp_user_group,$invalid,$user_id,$priority,$tat,$user,$new_user,$is_manual,$departmentName,$cmp_type,$refund_ammount,$premium_amount,$amount_claimed);
        ...
        { 
            $res = $objComplaint->ProgressComplaintVatality($login_id,$complaint_id,$progress,$notes,$cmode,$manual,$cmp_user_name,$cmp_user_group,$invalid,$reassign,$user_id,$priority,$tat,$user,$new_user,$is_manual,$departmentName,$cmp_type,$refund_ammount,$premium_amount,$amount_claimed);
        ...
        elseif($cmode == "bancaBank")
        {
            ...
            $res = $objComplaint->ProgressComplaintBancaBank($login_id,$complaint_id,$progress,$notes,$cmode,$manual,$cmp_user_name,$cmp_user_group,$invalid,$user_id,$priority,$tat,$user,$new_user,$is_manual,$departmentName,$cmp_type,$refund_ammount,$premium_amount,$amount_claimed);
    ...
    elseif($action == 'search_sms_interim')
    {
        ...
        // Example query — replace with your real table and columns
        $result    = $objComplaint->getSmsInterim($offset,$limit , $fromDate, $toDate);
        $rows = '';
        if (!empty($result)) {
            $rows = '';
            foreach ($result as $row) {
                $rows .= '
                        <tr>
                            ...
                            <td>'.htmlspecialchars($row['complaint_num']).'</td>
                            <td>'.htmlspecialchars($row['customer_name']).'</td>
                            <td>'.htmlspecialchars($row['customer_email']).'</td>
                            <td>'.htmlspecialchars($row['response_number']).'</td>
                            <td>'.htmlspecialchars($row['current_date']).'</td>
                            <td>'.htmlspecialchars($row['days_since_complaint']).'</td>
                            <td>'.htmlspecialchars($row['type']).'</td>
                        </tr>';
            }
            // Get total record count for pagination
            $countResult = $objComplaint->countSmsInterim($fromDate, $toDate);
            $totalRows = !empty($countResult) ? $countResult : 0;
