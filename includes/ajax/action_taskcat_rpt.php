                            ...
                            $output .='<tbody>';
                                $output .='<tr>';
                                    ...
                                    for($x=0; $x<count($data2); $x++)
                                    {
                                        ...
                                        array_push($ratingDataArr, $task_rating1);
                                        $task_rating_percetage1 = ($task_rating1[0]['task_within_tat'] / $task_rating1[0]['total_service_request']) * 100;

                                        // $getScoreCurrentMonth = $objTaskcatReport->getScoreCurrentMonth($deprtment_id, $month);

                                        //print_r($getScoreCurrentMonth);die;
                                        $count_ism_where_task_closed = count($data2);
                                    ...
                                    if($total_ism > 1)
                                    {
                                        $output .="<td rowspan=".$GradeCurrentMonth." rowspan=".$colspan1.">".$GradeCurrentMonth1."%</td>";
                                    }
                                    else
                                    {
                                ...
                                for($i=0; $i<count($data2); $i++)
                                {
                                    $output .='<tr>';
                                        if($data2 != 0) 
                                        {
                                            ...
                                            $task_rating_percetage = ($ratingDataArr[$i][0]['task_within_tat'] / $ratingDataArr[$i][0]['total_service_request']) * 100;
                                            ...
                                            $output .='<td>'.$ratingDataArr[$i][0]['total_closed_service'].'</td>';
                                        }
                                        else
                                        {
            ...
            $output .='<table id="tblTable" class="table table-igi table-responsive table-bordered">';
                        ...
                        $output .='<tfoot>';
                            ...
                            $output .='<tr>';
                                ...
                                $output .='<td colspan="11" class="text-center line-hight">'.number_format($SUM_Main_Hours/6,2).'</td>';
                            $output .='</tr>';
                        $output .='</tfoot>';
            $output .='</table>';
