...
if(isset($_POST)) 
{
    ...
    if (isset($_POST['action'])) 
    {
        ...
        elseif($action == 'update_progress')
        {
            ...
            echo $objTask->ProgressTask($login_id,$task_id,$progress,$notes,$policy,$priority,$ism,$assign_to);
        }
        elseif($action == 'upload')
        {
            ...
            $progress_status = isset($_POST['progress_status']) ? $_POST['progress_status'] : '';
            ...
            if(isset($_FILES['fileupload1']) && $_FILES['fileupload1']['size'] != 0)
            {
                ...
                $uploaddir = $dir.$task_num."/".$progress_status."__".$imagename;
        ...
        elseif($action == 'search_task')
        {
            ...
            $taskNumber   = isset($_POST['taskNumber'])?$_POST['taskNumber']: '';
            
            $group_id     = $_SESSION['group_id'];
            $user_type    = $_SESSION['user_type'];

            $data = $objTaskcat->SearchTask($login_id,$user_type,$group_id,$TaskCategory,$SubCategory,$txtISM,$tsk_status,$FromDate,$ToDate,$policy,$taskNumber);

            //print_r($data); die;

            $output ='';
            $output .='<table id="#task_grid" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th>Task ID</th>
                        ...
                        $output .='<tbody>';
                                foreach($data as $row)
                                {
                                    ...
                                    $deff = $objTask->GetDateTimeDiffHour($task_end_datetime, $current_datetime);
