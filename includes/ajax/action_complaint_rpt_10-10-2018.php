    ...
    if(isset($_POST['action'])) 
    {
        ...
        elseif($action == 'search_cmp_departmentwise_rpt')
        {
            ...
            $output .='<table id="tblTable" class="table table-igi table-responsive table-bordered">';
                        $output .='<thead>';
                            $output .='<tr>';
                                $output .='<th colspan="5">Complaint Details (Selected Duration)</th>';
                                $output .='<th colspan="5">Opened Cases Aging in Days (Selected Duration)</th>';
                            $output .='</tr>';
                            $output .='<tr>';
                                $output .='<th>Department <br>Name</th>';
                                $output .='<th>Complaints <br>Logged</th>';
                                $output .='<th>Complaints <br>Closed</th>';
                                $output .='<th>Complaints <br>Opened</th>';
                                $output .='<th>Overall <br>Percentage</th>';
                                $output .='<th>00 to 03 <br>Days</th>';
                                $output .='<th>04 to 06 <br>Days</th>';
                                $output .='<th>07 to 15 <br>Days</th>';
                                $output .='<th>16 to 30 <br>Days</th>';
                                $output .='<th>Above 30 <br>Days</th>';
                            $output .='</tr>';
                        $output .='</thead>';
                        ...
                        foreach($deprtments as $deprtment)
                        {
                            ...
                            $output .='<tbody>';
                                $output .='<tr>';
                                    ...
                                    $output .='<td>'.$sum_deprt_cmp_closed.'</td>';
                                    ...
                                    $output .='<td>'.$sum_deprt_cmp_opened.'</td>';

                                    $output .='<td>'.number_format(($sum_deprt_cmp/$all_sum_deprt_cmp)*100,2).'%</td>';

                                    $ageing = $objComplaintReport->getComplaintOpenCaseAgingByDepartId($deprtment_id,$durationType,$month,$quarter,$year,'03'); 
                                    $ageing_03 = $ageing[0]['AGEING_CMPL'] + $ageing[0]['AGEING_CMPC'] + $ageing[0]['AGEING_CMPLG'] + $ageing[0]['AGEING_CMPI'] + $ageing[0]['AGEING_CMPB'] + $ageing[0]['AGEING_CMPBB'] + $ageing[0]['AGEING_CMPV'];
                                    $cmpTotal03Days += $ageing_03;
                                    $output .='<td>'.$ageing_03.'</td>';


                                    $ageing = $objComplaintReport->getComplaintOpenCaseAgingByDepartId($deprtment_id,$durationType,$month,$quarter,$year,'06'); 
                                    $ageing_06 = $ageing[0]['AGEING_CMPL'] + $ageing[0]['AGEING_CMPC'] + $ageing[0]['AGEING_CMPLG'] + $ageing[0]['AGEING_CMPI'] + $ageing[0]['AGEING_CMPB'] + $ageing[0]['AGEING_CMPBB'] + $ageing[0]['AGEING_CMPV'];
                                    $cmpTotal06Days += $ageing_06;
                                    $output .='<td>'.$ageing_06.'</td>';

                                    $ageing = $objComplaintReport->getComplaintOpenCaseAgingByDepartId($deprtment_id,$durationType,$month,$quarter,$year,'15'); 
                                    $ageing_15 = $ageing[0]['AGEING_CMPL'] + $ageing[0]['AGEING_CMPC'] + $ageing[0]['AGEING_CMPLG'] + $ageing[0]['AGEING_CMPI'] + $ageing[0]['AGEING_CMPB'] + $ageing[0]['AGEING_CMPBB'] + $ageing[0]['AGEING_CMPV'];
                                    $cmpTotal15Days += $ageing_15;
                                    $output .='<td>'.$ageing_15.'</td>';


                                    $ageing = $objComplaintReport->getComplaintOpenCaseAgingByDepartId($deprtment_id,$durationType,$month,$quarter,$year,'30'); 
                                    $ageing_30 = $ageing[0]['AGEING_CMPL'] + $ageing[0]['AGEING_CMPC'] + $ageing[0]['AGEING_CMPLG'] + $ageing[0]['AGEING_CMPI'] + $ageing[0]['AGEING_CMPB'] + $ageing[0]['AGEING_CMPBB'] + $ageing[0]['AGEING_CMPV'];

                                    $cmpTotal30Days += $ageing_30;
                                    $output .='<td>'.$ageing_30.'</td>';

                                    $ageing = $objComplaintReport->getComplaintOpenCaseAgingByDepartId($deprtment_id,$durationType,$month,$quarter,$year,'A30'); 
                                    $ageing_A30 = $ageing[0]['AGEING_CMPL'] + $ageing[0]['AGEING_CMPC'] + $ageing[0]['AGEING_CMPLG'] + $ageing[0]['AGEING_CMPI'] + $ageing[0]['AGEING_CMPB'] + $ageing[0]['AGEING_CMPBB'] + $ageing[0]['AGEING_CMPV'];
                                    $cmpTotalA30Days += $ageing_A30;
                                    $output .='<td>'.$ageing_A30.'</td>';
                                $output .='</tr>';                                
                            $output .='</tbody>';
                        }
                        ...
                        $output .='<tfoot>';
                            $output .='<tr>';
                                $output .='<td>Total</td>';
                                $output .='<td>'.$cmpTotalLogged.'</td>';
                                $output .='<td>'.$cmpTotalClosed.'</td>';
                                $output .='<td>'.$cmpTotalOpened.'</td>';
                                $output .='<td>'.number_format($overallPercentage,2).'%</td>';
                                $output .='<td>'.$cmpTotal03Days.'</td>';
                                $output .='<td>'.$cmpTotal06Days.'</td>';
                                $output .='<td>'.$cmpTotal15Days.'</td>';
                                $output .='<td>'.$cmpTotal30Days.'</td>';
                                $output .='<td>'.$cmpTotalA30Days.'</td>';
                            $output .='</tr>';
                        $output .='</tfoot>';
            $output .='</table>';
