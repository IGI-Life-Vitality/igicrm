...
if(isset($_POST)) 
{
    ...
    if(isset($_POST['action'])) 
    {
        ...
        if($action == "save_lead")
        {
            ...
            $response = $objLead->SaveLeads($Salutation,$DepartmentName,$FName,$MName,$LName,$CNIC,$Gender,$Mobile,$BackNumber,$Email,$ddlCity,$Area,$Address,$ProductNameList,$CallTime,$Source,$LaedsDesc,$login_id);
             
            $result      = explode('|', $response);
            $res         = $result[0];     // success/fail
            $leadid      = $result[1];     // lead id     i.e. 1,2,3,4,5...
            $lead_number = $result[2];     // lead number i.e. LM181105001
            $send_mail   = $result[3];     // Regional Manager Email Address
            ...
            if($res == "success")
            {
                ...
                */
                ...
                if($ProductName == 1)
                {
                    $cc_emails = "mohsin.abbas@IGI.COM.PK";
                }
                else if($ProductName == 2)
                {
                    $cc_emails = "bakht.jamal@IGI.COM.PK,mohsin.abbas@IGI.COM.PK";
                }
                else if($ProductName == 3)
                {
                    $cc_emails = "akif.malik@IGI.COM.PK";
                }
                ...
                {
                    $cc_emails = "wasif.ali@IGI.COM.PK";
                }

                $subject = "Lead Acknowladgement by IGI Life";

                $sb .= "Thank you for contacting IGI Life Insurance. Your ticket number is $lead_number. Kindly use this for further queries. Our representative will get in touch with you shortly.\\nFor any further assistance, you can call 021 111 111 711";
                ...
                try
                {
                    ...
                    $to_email    = $Email;
                    $to_email_rm = $send_mail;
                    $subject     = "Lead Acknowladgement by IGI Life";

                    $mail->setFrom($email_username);

                    // Add Recipient/Recipients
                    if($Email != '')            
                    {
                        $mail->addAddress($to_email);     // Add a recipient CustomerName
                        $mail->addAddress($to_email_rm);  // Add a recipient RegionalManager
                    }
                    else
                    {
                        $mail->addAddress($to_email_rm);  // Add a recipient RegionalManager
        ...
        elseif($action == 'get_lead_user')
        {
            ...
            // $output .='<option value="'.$manager["id"].'"'. if($data[0]["id"] == $manager["id"]){ "selected='selected'" } > $manager["first_name"] . " " . $manager["last_name"]'</option>';
        ...
        elseif($action == 'search_lead')
        {
            ...
            //echo ($cnic."|".$lead_num."|".$product."|".$call_back."|".$FromDate."|".$ToDate);
            ...
            // echo $data = $objLead->SearchLead($lead_num,$cnic,$product,$call_back,$lead_status,$city,$FromDate,$ToDate);
