...
if(isset($_POST)) 
{
    ...
    if(isset($_POST['action'])) 
    {
        ...
        if($action == "save_lead")
        {
            ...
            // Comments for localhost testing - plz uncommnets before go to live
            ...
            // use "!=" for localhost testing - plz use "==" before go to live
            if($res == "success")
            {
                if($Email == '')
                {
                    $Email = "services.life@igi.com.pk";
                }

                /*
                    ------------------ LIST -> HEAD OF SALES ------------------
                    Product Name            ID (DB)      Email Address
                    Vitality                3            mohsin.abbas@IGI.COM.PK
                    Takaful                 2            mohsin.abbas@IGI.COM.PK
        ...
        elseif($action == 'update_lead_activty')
        {
            ...
            // Send email when new lead update
            // use "!=" for localhost testing - plz use "==" before go to live
