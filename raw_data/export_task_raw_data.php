<?php
    ...
    $objTask    = new Task();
    $data1      = $objTask->GetTask($login_id,$user_type,$group_id,$fromDate,$toDate);
    ...
    foreach($data1 as $row)
    {
        ...
        echo $csv_head = $row['task_num'].','.$task_status_id.','. str_replace(',','',$row['policy_number']).','.$cat[0]['fullname'].','.$subcat[0]['fullname'].','.$ism[0]['fullname'].','.preg_replace('/[\r\n]+/', " ", str_replace(',','',$row['task_desc'])).','.$row['priority'].','.$row['task_create_date'].','.$row['task_update_date'].','.$row['task_start_datetime'].','.$row['task_end_datetime'].','.$row['assignedTo'].','.$row['assignedBy'].','.(!empty($lastComment[0]['comments']) ? trim($lastComment[0]['comments']) : '')."\r\n";
    }
