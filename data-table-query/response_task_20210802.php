...
<?php
    ...
    else if($user_type == 4)
    {
    	$sql = "SELECT
			...
			FROM tbl_task_new t 
				...
				LEFT JOIN tbl_task_subcategory subcat ON subcat.id = t.task_subcat
				LEFT JOIN tbl_task_isam ism ON ism.id = t.task_ism 
			WHERE t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id'";
    }
    else
    {
    	$sql = "SELECT
			...
			FROM tbl_task_new t 
				...
				LEFT JOIN tbl_task_subcategory subcat ON subcat.id = t.task_subcat
				LEFT JOIN tbl_task_isam ism ON ism.id = t.task_ism 
			WHERE t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id' AND t.parent_task_id = '0'";
    }
	...
	//echo json_encode($sqlRec); die;
        // $t= 0;$pa = 5000;
   	//$sqlRec .=  " ORDER BY ". $columns[$params['order'][0]['column']]."   ".$params['order'][0]['dir']."  LIMIT ".$t." ,".$pa." ";
