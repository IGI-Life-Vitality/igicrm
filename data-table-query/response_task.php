<?php
	...
	$taskNumber   = isset($_POST['task_number'])?$_POST['task_number']: '';
	...
	if($TaskCategory != '')
		...
		if($taskNumber != '')
        {
            $whereAnd .= " AND t.task_num = '$taskNumber'";
        }

        if( !empty($params['search']['value']) ) {
			$whereAnd .= " AND usr.user_name LIKE '%".$params['search']['value']."%' ";
		}
	// echo json_encode($whereAnd);die;
    ...
    else if($user_type == 2)
    {
		...
		$whereGroup = " t.group_id IN ($group_id)";

		if (in_array(38, $session_group_id_array)) {
			$whereGroup = " t.channel = 'APP'";
		}
    	...
    	$sql = "SELECT
			...
			FROM tbl_task_new t 
				...
				LEFT JOIN tbl_task_subcategory subcat ON subcat.id = t.task_subcat
				LEFT JOIN tbl_task_isam ism ON ism.id = t.task_ism 
			WHERE $whereGroup $whereAnd";


		`echo "$sql" >> /tmp/query.log`;
    }
    else if($user_type == 4)
    {
    	$sql = "SELECT
    ...
    else
    {
    	$sql = "SELECT
			...
			FROM tbl_task_new t 
				...
				LEFT JOIN tbl_task_subcategory subcat ON subcat.id = t.task_subcat
				LEFT JOIN tbl_task_isam ism ON ism.id = t.task_ism 
			WHERE (t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id' OR t.task_verified_by = '$login_id')  $whereAnd";
    }//(t.task_assignee = '$login_id' OR t.task_assigned_to = '$login_id') AND t.parent_task_id = '0'
	...
	{ 
		...
		$nestedData[] = utf8_encode($task_num);							
		$nestedData[] = utf8_encode($task_status);						
		$nestedData[] = utf8_encode($row['policy_number']);				
		$nestedData[] = utf8_encode($row['cat']);				
		$nestedData[] = utf8_encode($row['subcat']);
		$nestedData[] = utf8_encode($row['ism']);
		$nestedData[] = utf8_encode($row['task_start_datetime']);
		$nestedData[] = utf8_encode($row['task_end_datetime']);
		$nestedData[] = utf8_encode($row['assignedTo']);
		$nestedData[] = utf8_encode($row['assignedBy']);
		$nestedData[] = utf8_encode($subtask);
		$data[] 	  = $nestedData;
	}
